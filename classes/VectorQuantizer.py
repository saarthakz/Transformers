...
class VectorQuantizer(nn.Module):
    def __init__(self, num_embeddings: int, latent_dim: int, beta: int) -> None:
        ...
        self.embedding = nn.Embedding(
            num_embeddings=self.num_embeddings, embedding_dim=self.latent_dim
        )
        self.embedding.weight.data.uniform_(
            -1.0 / self.num_embeddings, 1.0 / self.num_embeddings
        )  # Uniform data initialization for the codebook vectors
    ...
    def forward(self, z: torch.Tensor):
        ...
        z = z.permute(
            0, 2, 3, 1
        ).contiguous()  # Contigous method used for memory arrangement
        ...
        )  # Calculating the distances of the encoded vectors to the codebook vectors
