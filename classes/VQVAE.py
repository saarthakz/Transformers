...
class ResBlock(nn.Module):
    def __init__(self, in_channels: int, res_channels: int, out_channels: int) -> None:
        ...
        self.net = nn.Sequential(
            nn.ReLU(),
            nn.Conv2d(in_channels=in_channels, out_channels=res_channels, kernel_size=(3,3), stride=1, padding=1), # This Conv2D does not change the shape of the input, only the channels
            nn.ReLU(),
            nn.Conv2d(in_channels=res_channels, out_channels=out_channels, kernel_size=(3, 3), stride=1, padding=1) # This Conv2D does not change the shape of the input, including the channels. Try using the Conv Attention Block here instead
        )
...
class ResidualStack(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, res_channels: int, num_residual_layers: int):
        super(ResidualStack, self).__init__()
        layers = [ResBlock(in_channels, res_channels, out_channels) for _ in range(num_residual_layers)]
...
class Channelizer(nn.Module):
    def __init__(self, in_channels: int, out_channels: int) -> None:
        super().__init__()
        self.net = nn.Conv2d(in_channels, out_channels, kernel_size=(3, 3), stride=1, padding=1)
...
class Encoder(nn.Module):
    def __init__(self, image_channels: int, latent_dim: int, res_channels: int, num_residual_layers: int) -> None:
        super().__init__()
        ...
        layers.append(ResidualStack(latent_dim, latent_dim, res_channels, num_residual_layers))
...
class VQVAE(nn.Module):
    def __init__(self, image_channels: int, latent_dim: int, res_channels: int, num_residual_layers: int, vq_num_embeddings: int, beta: int, decay = 0) -> None:
        super().__init__()
        self.encoder = Encoder(image_channels, latent_dim, res_channels, num_residual_layers)
        self.decoder = Decoder(image_channels, latent_dim, res_channels, num_residual_layers)
        self.codebook = VectorQuantizer(vq_num_embeddings, latent_dim, beta) if decay == 0 else VectorQuantizerEMA(vq_num_embeddings, latent_dim, beta, decay)

        self.pre_quant_conv = nn.Conv2d(in_channels=latent_dim, out_channels=latent_dim, kernel_size=(1,1), stride=1, padding=0)

        self.post_quant_conv = nn.Conv2d(in_channels=latent_dim, out_channels=latent_dim, kernel_size=(1,1), stride=1, padding=0)
