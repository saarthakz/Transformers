        ...
        for epoch in range(args.epochs):
            with tqdm(range(len(train_dataset))) as pbar:
                for i, imgs in zip(pbar, train_dataset):
                    ...
                    disc_factor = adopt_weight(args.disc_factor, epoch*steps_per_epoch+i, threshold=args.disc_start)

                    perceptual_loss = self.perceptual_loss(imgs, decoded_images)
                    rec_loss = torch.abs(imgs - decoded_images)
                    g_loss = -torch.mean(disc_fake)

                    vq_loss = q_loss + disc_factor * g_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--numcodebook_vectors', type=int, default=256, help='Number of codebook vectors (default: 256)')
    parser.add_argument('--beta', type=float, default=0.25, help='Commitment loss scalar (default: 0.25)')
    parser.add_argument('--image_channels', type=int, default=3, help='Number of channels of images (default: 3)')
    parser.add_argument('--dataset_path', type=str, default='/data', help='Path to data (default: /data)')
    ...
    parser.add_argument('--disc_factor', type=float, default=1., help='Discriminator loss weighing factor')
